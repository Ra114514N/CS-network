...
# 引入数据配置
# 注意：确保 backend 目录下有 __init__.py，否则可能报模块导入错误
...
class DNSEngine:
    def __init__(self):
        # 内存缓存：key="domain|type", value=CacheEntry
    ...
    def _simulated_latency_ms(self) -> int:
    ...
    def _build_trace_step(self, server: str, level: str, qname: str, qtype: str, 
        ...
        return {
            "server": server,
            "level": level,  # client, root, tld, auth
            "qname": qname,
            "qtype": qtype,
            "response": response,
    ...
    def _resolve_authoritative(self, qname: str, qtype: str, use_lb: bool) -> dict:
        """模拟权威服务器查询"""
        # 简化逻辑：直接查 example.com 的 zone data
    ...
    def iterative_resolve(self, qname: str, qtype: str, config: dict) -> Tuple[dict, List[dict]]:
        """迭代查询逻辑：Local DNS 一步步去问 Root -> TLD -> Auth"""
        trace: List[dict] = []

        # 1. 检查本地缓存 (Local Cache)
    ...
    def recursive_resolve(self, qname: str, qtype: str, config: dict) -> Tuple[dict, List[dict]]:
        """递归查询逻辑：Client 只问 Local，Local 去跑腿"""
        trace: List[dict] = []
        latency = self._simulated_latency_ms()
